...
<div class="space-y-8 min-h-screen p-4 md:p-6" style="padding: 1rem;">
    ...
    {{-- ========== NORMAL IMPORT (<=5MB) – COMMENTED OUT ========== --}}
    {{--
    ...
    <div style="background-color: white; border-radius: 1rem; box-shadow: 0 1px 2px 0 rgba(0,0,0,0.05); border: 1px solid #f5f5f4; overflow: hidden; margin-top: 1.5rem;">
        ...
        <div style="padding: 1rem;">
            {{-- Row with file input and button aligned right --}}
            <div class="chunk-upload-row">
                <div class="file-input-wrapper">
                    ...
                    <p style="font-size: 0.75rem; color: #78716c; margin-top: 0.25rem;">No size limit – automatically splits into 2MB chunks</p>
                </div>
    ...
    {{-- Error display for normal import (if any) --}}
...
document.getElementById('uploadBtn').addEventListener('click', function() {
    ...
    reader.onload = function(e) {
        ...
        const chunkSizeLines = 2000; // प्रति chunk 2000 rows (अपनी जरूरत के हिसाब से बदलें)
        ...
        function sendChunk(index) {
            ...
            let chunkData;
            if (index === 0) {
                // First chunk: header + first batch of data
                chunkData = [header, ...dataLines.slice(start, end)].join('\n');
            } else {
                // Subsequent chunks: only data
            ...
            .then(data => {
                if (data.success) {
                    ...
                    resultDiv.innerHTML = `<div style="background:#fecaca;border:1px solid #dc2626;padding:0.75rem;border-radius:0.5rem;color:#7f1d1d;">${data.error}</div>`;
                }
            })
            .catch(err => {
                statusMsg.textContent = 'Network error: ' + err.message;
                progressBar.style.width = '0%';
                resultDiv.innerHTML = `<div style="background:#fecaca;border:1px solid #dc2626;padding:0.75rem;border-radius:0.5rem;color:#7f1d1d;">Network error: ${err.message}</div>`;
            });
        }
