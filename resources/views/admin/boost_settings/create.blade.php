...
@section('content')
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h4 class="mb-0 fw-bold" style="color: #0B1A20;">
                    <i class="fas fa-plus-circle me-2" style="color: #162E38;"></i>{{ __('admin.create_boost_plan_title') }}
                </h4>
                <p class="mt-2 mb-0" style="color: #6B7280;">{{ __('admin.create_boost_plan_subtitle') }}</p>
            </div>
            <div>
            ...
            <div class="col-lg-12">
                ...
                <div class="card border-0 shadow-lg rounded-3">
                    ...
                    <div class="card-body p-4">
                        <form action="{{ route('admin.boost-settings.store') }}" method="POST" id="createForm">
                            @csrf
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <div class="form-check form-switch">
                                            <input type="checkbox" name="is_active" class="form-check-input"
                                                ...
                                                style="width: 3rem; height: 1.5rem; background-color: #162E38; border-color: #162E38;">
                                            <label class="form-check-label ms-2 fw-bold" id="statusLabel"
                                                style="color: #162E38;">
                                                {{ __('admin.active_label') }}
                                            </label>
                                        </div>
                                ...
                                <div class="tab-pane fade" id="preview" role="tabpanel">
                                    ...
                                    <div class="card border-0 shadow-sm"
                                        ...
                                        <div class="card-body text-white p-4 text-center">
                                            ...
                                            <h3 class="fw-bold mb-2" id="previewDays">--{{ __('admin.day_boost') }}</h3>
                                            <div class="display-4 fw-bold mb-3" id="previewPrice">
                                                {{ __('admin.currency_symbol') }}0.00</div>
                                            <div class="mb-3">
                                                <span class="badge px-3 py-2"
                                                    style="background: rgba(255, 255, 255, 0.2); color: white;"
                                                    id="previewDaily">
                                                    {{ __('admin.currency_symbol') }}0.00/{{ __('admin.per_day') }}
                                                </span>
                                            </div>
                                            <div class="mt-3">
                            ...
                            <div class="d-flex justify-content-end gap-3">
                                ...
                                <button type="submit"
                                    ...
                                    <i class="fas fa-save me-2"></i>{{ __('admin.create_plan_btn') }}
                                </button>
                            </div>
                        </form>
                    </div>
    ...
    @push('scripts')
        <script>
            ...
            const dailyRatePreview = document.getElementById('dailyRatePreview');
            ...
            const dailyRateBox = document.getElementById('dailyRateBox');
            ...
            function updatePreview() {
                ...
                if (days > 0 && price > 0) {
                    ...
                    if (dailyRatePreview) dailyRatePreview.innerHTML =
                        `{{ __('admin.currency_symbol') }}${dailyRate.toFixed(2)}`;
                    previewDays.innerHTML = `${days}-{{ __('admin.day_boost') }}`;
                    previewPrice.innerHTML = `{{ __('admin.currency_symbol') }}${price.toFixed(2)}`;
                    previewDaily.innerHTML =
                        `{{ __('admin.currency_symbol') }}${dailyRate.toFixed(2)}/{{ __('admin.per_day') }}`;

                    if (dailyRateBox && noDataBox) {
                        dailyRateBox.style.display = 'block';
                        noDataBox.style.display = 'none';
                    }
                } else {
                    previewDays.innerHTML = '--{{ __('admin.day_boost') }}';
                    previewPrice.innerHTML = `{{ __('admin.currency_symbol') }}0.00`;
                    previewDaily.innerHTML = `{{ __('admin.currency_symbol') }}0.00/{{ __('admin.per_day') }}`;

                    if (dailyRateBox && noDataBox) {
                        dailyRateBox.style.display = 'none';
            ...
            if (statusSwitch) {
                statusSwitch.addEventListener('change', function() {
                    statusLabel.textContent = this.checked ? '{{ __('admin.active_label') }}' :
                        '{{ __('admin.inactive_label') }}';
                    statusLabel.style.color = this.checked ? '#162E38' : '#6B7280';
                });
            }

            // Trigger initial update to set correct state
            updatePreview();
        </script>
    @endpush
@endsection
