...
<style>
  ...
  * {
  ...
  .ornate-rule .l {
    flex: 1;
    height: 1px;
    background: linear-gradient(90deg, transparent, #b6903f);
  }
  .ornate-rule .l.r {
    background: linear-gradient(90deg, #b6903f, transparent);
  }
  .ornate-rule .d {
    width: 6px;
    height: 6px;
  ...
  .cover-content .divider-row .line {
    width: 70px;
    height: 1.5px;
    background: #eecd82;
  }

  .cover-content .divider-row .diamond {
    width: 8px;
    height: 8px;
    background: #eecd82;
    transform: rotate(45deg);
  ...
  /* ============================================================
     PRODUCT GRID — 4 columns × 2 rows (8 items per page)
     ============================================================ */
  .product-grid {
    width: 100%;
    margin: 0;
  ...
  .card-inner::before {
    left: 5px;
    border-left: 1.5px solid #b6903f;
  }
  .card-inner::after {
    right: 5px;
    border-right: 1.5px solid #b6903f;
  }
...
@php
    ...
    $companyData = [
        'tagline' => 'Lighting Beyond Ordinary',
...
@else
    <img class="back-logo" src="{{ public_path('logo/MORVOSKI-logoo.png') }}" alt="Morovski Logo">
@endif
...
<section class="page">
  ...
  <div class="toc">
    ...
    <div class="ornate-rule" style="margin-bottom:6px">
      <span class="l"></span><span class="d"></span><span class="l r"></span>
    </div>
    <ul class="toc-list">
...
<!-- ============================================================
     PAGES 4+: CATEGORY PAGES WITH PRODUCT GRID
     ============================================================ -->
    ...
    <div class="product-grid">
      ...
      <div class="product-card">
        <div class="card-inner">
          <div class="plate-number" style="color:{{ $categoryColors[$catPage['category_id']] ?? '#b6903f' }};">
            N&deg; {{ str_pad($loop->iteration, 2, '0', STR_PAD_LEFT) }}
          </div>
          ...
          <div class="meta-row">
            Ø {{ $item->specifications['Diameter'] ?? '—' }} · W {{ $item->specifications['Width'] ?? '—' }}<span class="sep">|</span>{{ $item->sku ?? 'N/A' }}
          </div>
...
@endforeach
    <!-- ============================================================
     BACK COVER (FIXED - FULLY POPULATED)
     ============================================================ -->
...
@endif
    ...
    <div class="line1">{{ $companyData['tagline'] ?? 'Illuminate Spaces, Elevate Style' }}</div>
    
    <div style="display: flex; align-items: center; justify-content: center; gap: 16px; margin: 10px 0 20px 0; width: 200px;">
      <span style="flex: 1; height: 1.5px; background: #eecd82;"></span>
      <span style="width: 8px; height: 8px; background: #eecd82; transform: rotate(45deg); flex-shrink: 0;"></span>
      <span style="flex: 1; height: 1.5px; background: #eecd82;"></span>
    </div>
    ...
    <div class="contact">
      ...
      @if($address)
        ...
        @if($address_line_2), {{ $address_line_2 }}@endif
        @if($city), {{ $city }}@endif
      ...
      @if($userEmail)
        {{ $userEmail }}<br><br>
      @else
        {{ $companyData['email'] ?? 'support@morovski.com' }}<br><br>
      @endif
      
      @if($isB2B)
        <b>Trade Enquiries</b><br>
        {{ $companyData['website'] ?? 'www.morovskilight.in' }} · {{ $companyData['social'] ?? '@Morvoski.lights' }}
      @else
        <b>Retail Enquiries</b><br>
        {{ $companyData['website'] ?? 'www.morovskilight.in' }} · {{ $companyData['social'] ?? '@Morvoski.lights' }}
      @endif
    </div>
    
    <div class="ornate-rule" style="margin-top: 30px; width: 250px;">
      <span class="l"></span>
      <span class="d"></span>
      <span class="l r"></span>
    </div>
    
    <div style="margin-top: 20px; font-family: 'Cormorant Garamond', serif; font-style: italic; font-size: 12px; color: rgba(238, 205, 130, 0.5); letter-spacing: 0.15em;">
      {{ date('Y') }} · {{ $companyName }}
      @if($userFullName)
        <br><span style="font-size: 10px;">Generated for {{ $userFullName }}</span>
      @endif
      @if($isB2B)
        <br><span style="font-size: 9px; color: rgba(238, 205, 130, 0.3);">B2B Account</span>
      @endif
    </div>
  </div>
</section>
